...
module Warspace {
    ...
    export class Ship extends eg.Collision.Collidable {
        ...
        constructor(payload: Server.IShipData, contentManager: eg.Content.ContentManager) {
            var lifeController = new ShipLifeController(payload);
            var levelManager = new ShipLevelManager(payload);

            var graphic = new ShipGraphic(payload.Name, payload.UserControlled, payload.Style, levelManager, lifeController, payload.MovementController.Position, payload.MovementController.Rotation, Ship.SIZE, contentManager);
