...
(function (Warspace) {
    var UserShipManager = (function () {
        function UserShipManager(ControlledShipId, _shipManager, _collisionManager, input, _camera, serverAdapter) {
            ...
            this._userCameraController = new Warspace.UserCameraController(this.ControlledShipId, this._shipManager, this._camera);
            this._lastSync = new Date();
            this.LatencyResolver = new Warspace.LatencyResolver(serverAdapter);
            this._collisionManager.OnCollision.Bind(function (ship, boundary) {
                if (ship instanceof Warspace.Ship && boundary instanceof Warspace.MapBoundary) {
                    if (ship.ID === _this.ControlledShipId) {
                        for (var i = Warspace.ShipMovementController.MOVING_DIRECTIONS.length - 1; i >= 0; --i) {
            ...
            this._shipInputController = new Warspace.ShipInputController(input.Keyboard, input.Mouse, function (direction, startMoving) {
    ...
    Warspace.UserShipManager = UserShipManager;
})(Warspace || (Warspace = {}));
