...
namespace Warspace
{
    public class Game
    {
        ...
        public const int SpawnsPerInterval = 2;
        ...
        private long Update(long id)
        {
            lock (_locker)
            {
                ...
                {
                    ErrorLog.Instance.Log(ex, "Error occured on Update");
                }
        ...
        private void UpdateLeaderboard(object state)
        {
            ...
            {
                ErrorLog.Instance.Log(ex, "Error occured on UpdateLeaderboard");
            }
        }
        ...
        public object initializeClient(string connectionId, RegisteredClient rc)
        {
            if (!UserHandler.UserExistsAndReady(connectionId))
            {
                        ...
                        if (user == null)
                        {
                            ...
                            else
                            {
                                ...
                                if (null != userData)
                                {
                                    ...
                                    RegisteredClient sqlRC = new RegisteredClient(Guid.NewGuid().ToString(), userData.UserID, userData.UserDisplayName, userData.UserProfilePicture);
                                    ship = new Ship(RespawnManager.GetRandomStartPosition(), GameHandler.BulletManager, userData);
                                    user = new User(userData.UserID, ship, sqlRC);
                                }
                                else
                                {
                                    // User was not found in the database
                                    // Create brand new user
                                    ship = new Ship(RespawnManager.GetRandomStartPosition(), GameHandler.BulletManager, rc.DisplayName);
                                    user = new User(connectionId, ship, rc) { Controller = false };

                                    // Add user entry into database
                                    Warspace.DataAccess.UserStore.CreateUserAsync(rc.Identity);
                                }
                ...
                {
                    ErrorLog.Instance.Log(ex, "Error on initializeClient in Game.cs");
                }
            }
        ...
        public object initializeController(string connectionId, RegisteredClient rc)
        {
            if (!UserHandler.UserExistsAndReady(connectionId))
            {
                ...
                {
                    ErrorLog.Instance.Log(ex, "Error on initializeController in Game.cs");
                }
            }
