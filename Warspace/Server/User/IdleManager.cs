...
namespace Warspace
{
    ...
    {
        private static readonly TimeSpan IDLE_AFTER = TimeSpan.FromSeconds(20); // Go idle after X seconds with no communication to the server
        private static readonly TimeSpan DISCONNECT_AFTER = TimeSpan.FromSeconds(42); // Disconnect after X hours of being idle
        ...
        public void GoIdle(DateTime now)
        {
            if (Idle == false)
            {
                ...
                {
                    _notificationManager.Notify("You've gone idle!  You will not see any new ships on screen.");
                }
        ...
        public void ComeBack()
        {
            if (Idle == true)
            {
                ...
                if (_me.Host.Connected)
                {
                    _notificationManager.Notify("Hey, you're Back!");
                }
